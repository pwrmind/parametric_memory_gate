...
# Делим на фазу обучения (80%) и фазу бэктестинга (последние 20% истории)
split = int(len(X) * 0.8)
X_train, X_test = X[:split], X[split:]
Y_train, Y_test = Y[:split], Y[split:]

# Индексы реальных цен для симуляции торгов на тест-участке
trade_prices = prices[seq_length + split:] 

# =====================================================================
# 3. ОБУЧЕНИЕ МОДЕЛИ НА ИСТОРИИ
# =====================================================================
...
# =====================================================================
# 4. СИМУЛЯЦИЯ ТОРГОВЛИ (БЭКТЕСТИНГ)
# =====================================================================
print("Запуск торгового робота на исторических тестах...")
model.eval()
with torch.no_grad():
    predictions_scaled = model(X_test).numpy()
    predictions = scaler.inverse_transform(predictions_scaled)

# Стартовые параметры симуляции
initial_balance = 1000.0  # Стартовый капитал в $
cash = initial_balance
crypto_inventory = 0.0    # Сколько BTC на балансе
in_position = False       # Сидим ли мы в активе
commission_rate = 0.001   # Комиссия биржи 0.1% за сделку
total_trades = 0

# Для метрики «Купил и держи»
buy_and_hold_btc = initial_balance / trade_prices[0][0]

# Порог триггера: прогнозируемый рост должен быть выше 0.5% для минимизации ложных сделок
...
for i in range(len(trade_prices) - 1):
    ...
    elif expected_return <= 0.0 and in_position:
...
# Финальная оценка капитала в последний день теста
final_price = trade_prices[-1][0]
if in_position:
    final_balance = crypto_inventory * final_price
else:
    final_balance = cash

buy_and_hold_final = buy_and_hold_btc * final_price

# =====================================================================
# 5. ОТЧЕТ О РЕЗУЛЬТАТАХ
# =====================================================================
print("\n" + "="*55)
print("📊 ОТЧЕТ СИСТЕМЫ БЭКТЕСТИНГА ТОРГОВОГО БОТА")
print("="*55)
...
if final_balance > buy_and_hold_final:
    print("🏆 РЕЗУЛЬТАТ: Бот на вашей памяти ОБОШЕЛ рынок!")
else:
    print("📉 РЕЗУЛЬТАТ: Пассивное удержание оказалось выгоднее в данном цикле.")
